...
tic
vec_opttm=60:-1:30;%0:0.2:1;
for opttmval=21:length(vec_opttm)
global np K Ct T N phi Phivec %MVL MVC L C MLV MCV V
% phi=vec_opttm(opttmval);
max_t=180; % Maximum time  TO BE GIVEN
dt=max_t/5;
...
arr_mx_t=max_t*5/5;%[max_t*1/5;max_t*3/5;max_t*5/5];%;350;400;500;600;700];
...
%     else
        ...
        t_pphge(2*k)=t_pphge(2*k-1)+seq_prophage(k)*(tmax/np-vec_opttm(opttmval));%/k;%[0 tmax/np 2*tmax/np 3*tmax/np 4*tmax/np tmax];
%     end
end
% t_pphge(2*k+1)=tmax;
j=1;
...
%     for i=1:length(X0)
%         if and(SOL_i(end,i)>0,SOL_i(end,i)<1)
%             rnd=rand(1);%%%CHECK IT
...
G(opttmval,1:2^(np-1))=F(1,1:2^(np-1));
clearvars -except vec_opttm opttmval G
end
bX=180./vec_opttm;
