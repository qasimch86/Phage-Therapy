...
tic
vec_alpha=10^-6:10^-5/5:10^-5;%0:0.2:1;
for alphaval=1:length(vec_alpha)
global np K Ct T N alpha Phivec phi %MVL MVC L C MLV MCV V
alpha=vec_alpha(alphaval);
...
arr_mx_t=max_t*1/5;%[max_t*1/5;max_t*3/5;max_t*5/5];%;350;400;500;600;700];
for tm=1:length(arr_mx_t)
    Parameters();
    tmax=arr_mx_t(tm);
    fprintf('Maximum time: %d \n',tmax);
for num_seq=3:2^(np-1) % Simulations/Prophage sequence Number
...
G(alphaval,1:2^(np-1))=F(1,1:2^(np-1));
clearvars -except vec_alpha alphaval G
end
