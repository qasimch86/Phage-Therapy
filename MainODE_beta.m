...
tic
% vec_beta=[10^-6:10^-6:10^-5;%0:0.2:1;
vec_beta=[10^-11:10^-11:10^-9 10^-9+10^-10:10^-10:10^-8 10^-8+10^-9:10^-9:10^-7];%2*10^-6:10^-6:10^-5 2*10^-5:10^-5:10^-4];%10^-10:10^-10:10^-9 2*10^-9:10^-9:10^-8 2*10^-8:10^-8:10^-7 
for betaval=1:length(vec_beta)
global np K Ct T N phi Phivec beta %MVL MVC L C MLV MCV V
beta=vec_beta(betaval);
...
arr_mx_t=max_t*3/5;%[max_t*1/5;max_t*3/5;max_t*5/5];%;350;400;500;600;700];
...
%     else
        ...
        t_pphge(2*k)=t_pphge(2*k-1)+tmax*seq_prophage(k)/np/4;%/k;%[0 tmax/np 2*tmax/np 3*tmax/np 4*tmax/np tmax];
%     end
end
% t_pphge(2*k+1)=tmax;
j=1;
...
for k=1:length(t_pphge)-1
    ...
    if t_pphge(k) == t_pphge(k+1)
        ...
        [t_i,SOL_i]=ode15s('ODE_sol3',[t_pphge(k) t_pphge(k+1)],X0);
    for i=1:length(X0)
...
G(betaval,1:2^(np-1))=F(1,1:2^(np-1));
clearvars -except vec_beta betaval G
betaval
pause(1);
end
% Fig_endpoint([1 2 3], F)
figure;plot(vec_beta,G);
toc